// ─────────────────────────────────────────────────────────────────────────────────────────────────────
// RETIRED 2026-09-14 — wearables strategy decision D6 (`.context/research/2026-09-14_wearables-implementation-strategy.md`).
//
// This is the Garmin adapter as it lived at `_shared/wearables/garmin.ts` from 2026-09-04 to 2026-09-14,
// copied VERBATIM below this header (the `./core.ts` import is relative to its ORIGINAL location and is
// intentionally left as it was — nothing imports this file; `_retired/` sits outside every function and
// outside `_shared/`, so no bundle and no `deno check` of a function reaches it).
//
// Why it was retired
//   • Every endpoint, scope, token lifetime and payload shape in it was coded from old mirror/community
//     docs, not from Garmin's approved-partner documentation. The corpus (`vendors/garmin.md`) marks all
//     of these PORTAL-GATED and says: "Do not code guessed endpoints."
//   • `parseWebhook` "verified" pushes by comparing the `garmin-client-id` header with our client id.
//     That is not a signature — the client id is not a secret — so the webhook endpoint accepted any
//     body from anyone who knew (or guessed) it. `20_WEBHOOK_SYNC_SECURITY.md`: "Do not verify only by
//     a client-id header based on old mirror docs."
//   • The header claimed the Connect Developer Program was "closed to new applicants". The pack's
//     correction (`23_VENDOR_STATUS_AND_OWNER_STEPS.md`): the 2026 program pages invite *reviewed
//     business applicants*; FunctionAlps simply has no approved contract yet.
//
// What is worth keeping from it
//   `mapItem()` — the summary-type → catalogue-row mapping (dailies, sleeps, hrv, pulseOx, respiration,
//   stressDetails, epochs, userMetrics, bodyComps, skinTemp, bloodPressures, activities, mct). It is a
//   reasonable starting point ONCE the field names, units and scales are confirmed against the approved
//   schema. Treat every field in it as UNVERIFIED until then (D4: `lastNightAvg` is raw-only).
//
// What must be true before any of this comes back into `_shared/wearables/garmin.ts`
//   1. An approved Garmin Connect Developer Program contract for FunctionAlps, with the official API /
//      OpenAPI docs exported into the repository (corpus §3 step 5).
//   2. OAuth authorize / token / refresh / revoke endpoints, PKCE mode and scopes taken from those docs.
//   3. A REAL webhook authentication scheme from the approved docs (a signature or equivalent secret-
//      bearing mechanism) — never a client-id header comparison.
//   4. Sanitised official payload samples copied into `tests/fixtures/` and contract tests generated from
//      them (corpus §22, §29). No fabricated payloads.
//   5. The legal / AI-use review of the commercial licence (corpus §27; `ai-policy.ts` keeps Garmin denied).
// ─────────────────────────────────────────────────────────────────────────────────────────────────────
