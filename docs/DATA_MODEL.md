...
| `TodayFocus`, `FocusOffer` | `habit_offers` (+ `rank`, `reason`, `pillar`, `slot`, `variant`, `superseded` — migration `20260925_habit_offers_daily_focus.sql`), read via `member-daily-focus`; content from `state_responses` (practice-written, per state) and `habit_bank` (easy · standard · progression) | one row per offer per day (UNIQUE patient · offer_key · offered_on); a recomputed day retires rows (`superseded`), never deletes; members may update today's rows only | **live on iOS 2026-09-25** — the web habit UI is still flagged off |
