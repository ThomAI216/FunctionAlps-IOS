...
| `MealReaction` | `nb_meal_reactions` | `meal_log_id`, `overall` (= the digestion read), `digestion`, `energy`, `bloating`, `fullness`, `gas_burden`, `burning` (reflux), `fatigue`, `responses` (carries `focus`, which has no column), `reaction_flags[]`, `reaction_time` | Phase D |
...
| `CheckinMoment` | `patient_checkin_moments` | `checkin_date`, `slot` (morning/evening; `midday` is retired — read-only legacy), markers, `sleep_bed_time` / `sleep_wake_time` (wall clock, `time`), `pills` jsonb, `note`; UNIQUE (patient, date, slot); writes limited to today/yesterday | Phase B |
...
| `HabitPlan`, `HabitRow`, `HabitCompletionRow`, `HabitPlanHeader`, `HabitPlanPhase`, `HabitAction` | `care_plans` (`objective_line`), `care_plan_phases` (patient-renderable columns only — never `gate_criteria`), `habits` (RLS: own, not cancelled, prescribed only when the item is patient-visible and approved), `habit_completions` (`completion_date`, `logged_by='patient'`, member INSERT/DELETE today ± 1 patient-local) | `HabitEngine` = the Expo `lib/plan` arithmetic; `evaluate-gates` poked after a check-off | **live on iOS 2026-09-25** (Home card) — `habit_notes`, `care_plan_item_gates`, supplements not yet |
| `TodayFocus`, `FocusOffer` | `habit_offers` (+ `rank`, `reason`, `pillar`, `slot`, `variant`, `superseded` — migration `20260925_habit_offers_daily_focus.sql`; + `title_fr`, `description_fr` — `20260925_focus_offer_texts_fr.sql`), read via `member-daily-focus`; content from `state_responses` (practice-written, per state; `title_fr`, and `title_fr` / `description_fr` inside each `offers` element) and `habit_bank` (easy · standard · progression, each with its `*_fr`) | one row per offer per day (UNIQUE patient · offer_key · offered_on); a recomputed day retires rows (`superseded`), never deletes; members may update today's rows only | **live on iOS 2026-09-25** — the web habit UI is still flagged off |
