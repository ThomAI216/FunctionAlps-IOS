...
| Today's focus | `POST {SUPABASE_URL}/functions/v1/member-daily-focus` `{recompute?}` → `{day, needsCheckin, offers[]}` | this repo — `_shared/focus/engine.ts` decides (pure, deterministic, no LLM per `ai-policy.ts`); runs under the member's session, RLS on every read/write | member bearer (verify_jwt ON) |
